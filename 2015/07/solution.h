 ...
 * inputs have arrived. finally, a signal value can be given to a wire
 * (turn the circuit on) to start the process.
 */

/*

  this is not a program, it is a circuit. gates don't process and send
  until all inputs arrive. notifications could be fun.
...
/*
 * a gate has one or more inputs and one output. for and/or,
 * regardless of order of input (16 AND w verse x OR 8) the wire
 * identifier goes to wire 1. wire 2 being an empty string means use
 * the mask value.
 */
 ...
 * connection structure holds all the possibilities, with the
 * source_type and somtimes gate_type determining which fields are
 * meaningful in a particular connection.
 */
...
struct connection {
	...
	gate inp_gate;                   /* if it's a gate, it's described in here */
...
/*
 * a circuit definition arbitrarily has a max of 512 connections. as a
 * wire can have only one input, there should be no duplicates.
 *
 ...
 * dataset there are two of these among the roughly 300
 * connections.
 */
...
bool
add_connection(
        circuit *,           /* address of the circuit board */
        const char *         /* input string from dataset to parse and add */
);


/*
 * parse an input line producing an intermediate block. this is
 * exposed here for unit testing, connections should be added through
 * the prior addConnection function. any error is reported by setting
 * the output connection wire name to an empty string.
 */
...
/*
 * flip the on switch to run the circuit. returns false the circuit
 * failed.
 */
...
/*
 * the code for each of the day's parts. each expects the name of an
 * input dataset or some other string to drive the part's code. the
 * answer for the posed problem is printed to stdout. these return
